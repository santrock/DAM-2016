		...
		success: function(data){
				...
				for (var i = 0; i < votos.length; i++) {
					if(votos[i].voto == filtro){
						...
						totalInsertados++;
						if(totalInsertados>134 && totalInsertados<170){
							$("#SceneScene3aF3 .cuadroDiputado").append('<div class="diputado" idDip="'+votos[i].asiento+'">\
...
SceneScene3aF3.prototype.handleKeyDown = function (keyCode) {
	...
	switch (keyCode) {
		...
		case sf.key.DOWN:
			sf.scene.hide('Scene3aF3');
			sf.scene.show('Scene3aF4');
			sf.scene.focus('Scene3aF4');
			break;
		case sf.key.ENTER:
			break;
		case sf.key.RETURN:
